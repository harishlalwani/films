<?php
	...
	class UploadComponent extends Component {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('gif','jpeg','png','jpg','doc','docx','xls','xlsx','pdf','txt','csv'); }
		...
		function upload_image_from_url($array=array())
		{
			if(isset($array['url']) && $array['url'] && isset($array['save_location']) && $array['save_location'])
			{
				$profile_url = file_get_contents($array['url']);
				$image_name = time().'.jpg';
				file_put_contents($array['save_location'].$image_name,$profile_url);
				return array("success"=>true,"error"=>false,"message"=>"Image uploaded successfully","image"=>$image_name);
			}
			else
			{	$message = "";
				if(!isset($array['url']) || empty($array['url']))
				$message = "Image url not provided";
				else if(!isset($array['save_location']) || empty($array['save_location']))
				$message = "File Location not provided";
				return array("success"=>false,"error"=>true,"message"=>$message);
			}
		}
		
		function is_file_exist($array=array())
		{
			if(file_exists($array['dir'].$array['file']))
			return true;
			else
			return false;
		}
